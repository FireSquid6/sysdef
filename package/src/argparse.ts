...
> {
  ...
  static create() {
    return new FlagSet([] as const, [] as const);
  }

  flag<TName extends string, TType extends keyof TypeMap = 'boolean'>(
    long: TName, 
    config: Omit<FlagConfig<TType>, 'long'> = {} as any
  ): FlagSet<[...TFlags, FlagConfig<TType> & { long: TName }], TOptions> {
    return new FlagSet([...this.flags, { long, ...config }] as const, this.options);
  }
