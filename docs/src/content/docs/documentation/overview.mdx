...
Each [Module](/guides/modules) is a set of directories, files, and packages you want installed, created, and managed on your machine. You also have [Providers](/guides/providers) which provide an interface for interacting with a package manager. Some default providers for the package managers `apt`, `arch-official`, `aur`, `dnf`, `bun`, `npm`, `pipx`, `go`, and `cargo` are included. It's expected that you have the ones you use installed already.
